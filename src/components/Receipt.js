...
function Receipt() {
  ...
  const amount = Number(100000000000 - total);
  const { val } = useSpring({ from: { val: 0 }, to: { val: amount } });

  return (
    <div className="receipt">
      <div className="box">
        <div className="summarydiv">
          <div style={{ marginBottom: "25px" }}>Your Receipt</div>
          <div className="summarybottom">
            {products.map((item) =>
              item.number > 0 ? (
                <ul className="summary">
                  <li className="first">{item.title}-</li>
                  <li className="middle">X{item.number}</li>
                  <li className="last">${item.total}</li>
                </ul>
              ) : null
            )}
          </div>
